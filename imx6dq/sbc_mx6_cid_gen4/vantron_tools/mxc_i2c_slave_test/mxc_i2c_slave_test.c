...
#define I2C_MASTER_DEV "/dev/i2c-1"
...
static int deal_master();
static int deal_slave();

static void usage()
{
	printf("Usage:\n\t./mxc_i2c_slave_test [-m | -s]\n\n");
}
...
int main(int argc, char *argv[])
{
	...
	c = getopt(argc, argv, "ms");
	switch (c) {
	case 'm':
		ret = deal_master();
		if (ret < 0)
			printf("i2c deal_master fail!\n");
		break;
	case 's':
		ret = deal_slave();
		if (ret < 0)
			printf("i2c deal_slave fail!\n");
		break;
	default:
		usage();
		break;
