 ...
 *
 ******************************************************************************
 * $Id: suplSMmanager.cpp 114241 2016-05-02 14:16:43Z fabio.robbiani $
 * $HeadURL: http://svn.u-blox.ch/GPS/SOFTWARE/hmw/android/release/v3.30/gps/supl/suplSMmanager.cpp $
 *****************************************************************************/
    ...
    while (pHandler != NULL)
    {
		...
		else if (pHandler->state == RRLP_RESP)
		{
			if (pHandler->responseType == RESP_POSITION_DATA)
			{
				...
				/*UBX_LOG(LCAT_VERBOSE, "Pos response pending - PA %i  SA %i  Acc %i  RHAcc %i  RVAcc %i",
					       posAvail, speedAvail, accuracy, pHandler->reqHorAccuracy, pHandler->reqVerAccuracy);
				*/
				if ((posAvail) &&
					(speedAvail) &&
				...
				{
					UBX_LOG(LCAT_VERBOSE, "Position available with accuracy %d, lat %f, lon %f, speed %f",
			...
			{
				/*UBX_LOG(LCAT_VERBOSE, "MSA response still pending - delay %ld - TO %ld",
...
void suplHandleAuthorization(int sid, int cmd)
{
    ...
    {
        UBX_LOG(LCAT_ERROR, "Not in the main Thread (%.8X %.8X)",
...
static void processRawSuplMessage(const char *buffer, int size, suplHandler_t *pReceivedHandler)
{
	...
	UBX_LOG(LCAT_VERBOSE, "Msg decoded. Msg type %i(%s)",
		 pMsg->message.present, _LOOKUPSTR(pMsg->message.present, UlpMessageType));
	...
	if (pMsg->message.present == UlpMessage_PR_msSUPLINIT)
	{
		...
		else
		{
			...
			else
			{
                ...
                UBX_LOG(LCAT_VERBOSE, "%d NiMsg saved here %p in thread %u", __LINE__, pMsg, (unsigned int) pthread_self());
...
static int suplSM(suplHandler_t *pHandler, struct ULP_PDU *pMsg, SuplCmd_t cmd)
{
    ...
    UBX_LOG(LCAT_VERBOSE, "SM entry with Id = %d in state %s (%d) with command %d", pHandler->sid,stateInfo[pHandler->state],pHandler->state,cmd);
    ...
    case START:
        ...
        else
        {
            ...
            if (pMsg->message.present == UlpMessage_PR_msSUPLINIT)
            {
				...
				logAgps.write(0x02000000, "%d, 1.0.0, SUPL_INIT # SUPL_MODE : NI_%s", pHandler->sid,
                ...
                {
                    UBX_LOG(LCAT_VERBOSE, "notification type %ld", pMsg->message.choice.msSUPLINIT.notification->notificationType);
        ...
        else
        {
            ...
            if (pMsg->message.present == UlpMessage_PR_msSUPLEND)
            {
                ...
                if (pMsg->message.choice.msSUPLEND.statusCode != NULL)
                {
					...
					logAgps.write(0x02000000, "%d, 1.0.0, SUPL_END # SUPL_MODE : %s", pHandler->sid,
			...
			else
			{
				...
				logAgps.write(0x02000000, "%d, 1.0.0, SUPL_RESPONSE # SUPL_MODE : SI_%s", pHandler->sid,
				...
				if (pMsg->message.choice.msSUPLRESPONSE.sLPAddress != NULL)
				{
					...
					UBX_LOG(LCAT_VERBOSE, "Can not handle because non proxy mode expected but not supported (%i)",
                ...
                else if (!verifySetSessionId(pMsg->sessionID.setSessionID, pHandler->pSetId))
                {
					...
					UBX_LOG(LCAT_VERBOSE, "wrong sid %d inseat of %d", pHandler->sid, (int) pMsg->sessionID.setSessionID->sessionId);
					UBX_LOG(LCAT_VERBOSE, "Sending SUPLEND ====>");
				...
				{
					UBX_LOG(LCAT_VERBOSE, "Processing SUPLRESPONSE - Pos Method %li(%s)",
						 posMethod, _LOOKUPSTR((U) posMethod, PosMethodType));
					
					if ((posMethod == PosMethod_agpsSETbased) ||
						(posMethod == PosMethod_agpsSETassisted))
					{
						UBX_LOG(LCAT_VERBOSE, "Handling SUPLRESPONSE (%s)",
...
//						par.hash = 0x1122334455667788LL;
        ...
        if (cmd == SUPL_AUTH_GRANT)
        {
            ...
            else
            {
                ...
                else
                {
					...
					if ((pDatabase->GetOutput(CDatabase::DATA_LATITUDE_DEGREES, lat) && pDatabase->GetOutput(CDatabase::DATA_LONGITUDE_DEGREES, lon)) &&	
    ...
    case RRLP:
        ...
        {
			UBX_LOG(LCAT_VERBOSE, "Msg from network: Present %i(%s)",
				 pMsg->message.present, _LOOKUPSTR(pMsg->message.present, UlpMessageType));
			...
			if (pMsg->message.present == UlpMessage_PR_msSUPLPOS)
			{
				...
				logAgps.write(0x02000000, "%d, 1.0.0, SUPL_POS # SUPL_MODE : %s : recv",
							  pHandler->sid,
							  pHandler->networkInitiated ? "NI" : "SI");
...
/*
					if ((pHandler->networkInitiated) && (pHandler->assistanceRequested) && (!aux.assistDataReceived))
					{
						...
						UBX_LOG(LCAT_VERBOSE, "Assistance data not received");
						UBX_LOG(LCAT_VERBOSE, "Sending SUPLEND ====>");
...
*/
					...
					else
					{
						...
						logAgps.write(0x02000000, "%d, 1.0.0, SUPL_POS # SUPL_MODE : %s",
									  pHandler->sid,
									  pHandler->networkInitiated ? "NI" : "SI");
			...
			else if (pMsg->message.present == UlpMessage_PR_msSUPLEND)
			{
                ...
                if (pMsg->message.choice.msSUPLEND.statusCode != NULL)
                {
					...
					logAgps.write(0x02000000, "%d, 1.0.0, SUPL_END # SUPL_MODE : %s",
								  pHandler->sid,
								  pHandler->networkInitiated ? "NI" : "SI");
...
*/
    ...
    UBX_LOG(LCAT_VERBOSE, "SM exit with Id = %d in state %s (%d) with value %d", pHandler->sid,stateInfo[pHandler->state],pHandler->state, res);
