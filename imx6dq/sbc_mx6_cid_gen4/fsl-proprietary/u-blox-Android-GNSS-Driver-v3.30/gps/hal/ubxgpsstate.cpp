...
#define SERPORT_DEFAULT             "/dev/ttymxc3" //!< Default receiver interface
...
//lamson check gps pwr state
#define GPS_PWR_SYSENTRY "/sys/class/gpio/gpio104/value"
#define GPS_RST_SYSENTRY "/sys/class/gpio/gpio38/value"
...
bool CUbxGpsState::powerOn(void)
{
	...
	/* power it up */
	snprintf(cmd, sizeof(cmd), "echo 1 > %s", GPS_PWR_SYSENTRY);
	system(cmd);

	// reset the gps receiver
	snprintf(cmd, sizeof(cmd), "echo 0 > %s", GPS_RST_SYSENTRY);
	system(cmd);
	usleep(100000);
	snprintf(cmd, sizeof(cmd), "echo 1 > %s", GPS_RST_SYSENTRY);
	system(cmd);
	
	LOGV("lamson--> GPS Poweron");
	return true;
}
...
void CUbxGpsState::powerOff(void)
{
	...
	char cmd[128];
	/* power it down */
	snprintf(cmd, sizeof(cmd), "echo 0 > %s", GPS_PWR_SYSENTRY);
	system(cmd);
	LOGV("lamson--> GPS Poweroff");
}
